...
static void init(scicos_block *block)
{
    ...
    UDP_num_tx_channels = GetIparVal(block, 1);
}
...
static void inout(scicos_block *block)
{
    ...
    for(i=0; i < UDP_num_tx_channels; i++) {
        void *in_ptr = GetInPortPtrs(block, i+1);
        memcpy(scicosUDP_tx_buffer, in_ptr, sizeof(UDP_message) );
        UDP_Buffer_Putmsg(&UDP_tx_buffer, scicosUDP_tx_buffer);
    }
}
