...
/* This source file has been modified to make the stack working with
 * the flex demoboard and Erika RTOS.
 *
...
#include "console_serial.h" //GF

//Start GF
#define printf(x)	 GetResource(S1);\
					 console_write(MY_FIRST_CONSOLE, (unsigned char *)x, strlen(x));\
					 ReleaseResource(S1);\
...
// GF: ifdef statement added to avoid warning when ENABLE_BROADCAST is not defined
#ifdef ENABLE_BROADCAST
DiscardPacketHere:
#endif
...
BYTE AddConnection(void)
{
    ...
    else
    {
        ...
        /*
         * GF: looking at the definition of struct P2P_CONNECTION_ENTRY in P2P.h,
         * the member IncomingFrameCounter is not defined when TARGET SMALL it is.
         * So, we have modified the following #ifdef statement accordingly
         *
         */
        //#ifdef ENABLE_SECURITY
		#if defined(ENABLE_SECURITY) && !defined(TARGET_SMALL)
