...
static void SyncENCPtrRAWState(UINT8 encPtrId)
{
    ...
    if ( g_encIndex[encPtrId] < BASE_TCB_ADDR )
    {
		...
		#if defined(DEEP_RS_STACK_USE_UART) //xxxx
		putrsUART("RS: rx in progress?\r\n");
		#endif


        if ( g_encIndex[encPtrId] < TXSTART )
        {
			#if defined(DEEP_RS_STACK_USE_UART) //xxxx2
			putrsUART("RS: < TXSTART\r\n");
			#endif
        ...
        if (GetRawWindowState(rawId) != WF_RAW_DATA_MOUNTED) 
        {
			...
			#if defined(DEEP_RS_STACK_USE_UART) //xxxx2
			putrsUART("RS: buffer not mounted\r\n");
			#endif
...
BOOL MACGetHeader(MAC_ADDR *remote, BYTE* type)
{
    ...
    if ( GetRawWindowState(RAW_RX_ID) == WF_RAW_DATA_MOUNTED )
    {
		...
		#if defined(DEEP_RS_STACK_USE_UART) //xxxx!!
		putrsUART("RS: buffer to save - mac header\r\n");
		#endif
