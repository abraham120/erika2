...
void TickInit(void)
{
...
#else
	// Use Timer 6 for 16-bit and 32-bit processors
	// 1:256 prescale
	T6CONbits.TCKPS = 3;
	// Base
	PR6 = 0xFFFF;
	...
	#if defined(__C30__)
		IPC11bits.T6IP = 2;	// Interrupt priority 2 (low)
		IFS2bits.T6IF = 0;
	#else //TODO: change the following to use timer 6
		#error    Check if timer 6 exist in your PIC (see Tick.c)
