...
COMPILER_INLINE void touch_2_calibrate(tune_raw_t *t)
{
	int32_t tmp;
		...
		uint16_t touch_range)
{
	...
	if(is_raw_initialized)
		touch_2_calibrate(&tune_raw);

	return TOUCH_ERROR_NONE;
}

COMPILER_INLINE void touch_2_set_adc(void)
{
	CLEARBIT(EE_2_TOUCH_EN_A_TRIS);
	CLEARBIT(EE_2_TOUCH_EN_B_TRIS);
	CLEARBIT(EE_2_TOUCH_EN_C_TRIS);
	SETBIT(EE_2_TOUCH_BOTTOM_TRIS);
	SETBIT(EE_2_TOUCH_RIGHT_TRIS);
	CLEARBIT(EE_2_TOUCH_BOTTOM_AD);
	CLEARBIT(EE_2_TOUCH_RIGHT_AD);

	STANDBY_2_CONFIGURATION;
	tf.STANDBY = 1;

	ADC_2_SELECTED_PIN = STANDBY_PIN_2;
	SETBIT(ADC_2_ASAM);
	ADC_2_FORM = 0;
	ADC_2_SSRC = 0;
	SETBIT(ADC_2_AD12B);
	...
	CLEARBIT(ADC_2_INTERRUPT_FLAG);
	// enable ADC interrupts, disable this interrupt if the DMA is enabled
	SETBIT(ADC_2_INTERRUPT_ENABLE);
	// turn on ADC
	ADC_2_TURN_ON;
}

COMPILER_INLINE void touch_2_set_act_time(uint16_t _touch_tick_us)
{
...
void touch_2_raw_init(void)
{
	...
	touch_2_set_adc();
	touch_2_set_act_time(TOUCH_ACTIVATION_TIME);
	touch_2_kal_init(touch_tick_us);

	is_raw_initialized = 1;
}

void touch_2_tune(tune_raw_t *t)
{
	tune_raw = *t;
	touch_2_calibrate(&tune_raw);
	is_tuned = 1;
...
{
	if(ADC_2_CONVERSION_DONE)
	{
		ADC_2_CONVERSION_RESET;
		if(tf.STANDBY)
		{
			Reading_low_level = ADC_2_RESULT_BUFFER;
			if(Reading_low_level < MAXIMUM_LOW_LEVEL)
			{
				...
				Y_POS_2_CONFIGURATION;
				ADC_2_SELECTED_PIN = ADC_Y_2;
				untouch_conditions = 0;
			} else
			{
				tf.COMPLETE = 0;
		...
		{
			Reading_Y[i_array] = ADC_2_RESULT_BUFFER;
			tf.YPOS = 0;
			tf.XPOS = 1;
			X_POS_2_CONFIGURATION;
			ADC_2_SELECTED_PIN = ADC_X_2;
		} else if(tf.XPOS)
		{
			Reading_X[i_array] = ADC_2_RESULT_BUFFER;
			tf.XPOS = 0;
			tf.STANDBY = 1;
			tf.COMPLETE = 1;
			STANDBY_2_CONFIGURATION;
	...
	{
		ADC_2_CONVERSION_START;
	}
}
#endif // __LOW_LEVEL_MEASUREMENT__

ISR2(ADC_2_INTERRUPT_NAME)
{
	CLEARBIT(ADC_2_INTERRUPT_FLAG);
	ActivateTask(TASK_TOUCH_MANAGER_2);
}

int8_t touch_2_poll_raw_position(
		uint8_t touch_axis,
		int16_t *raw_choord)
{
	if(!is_raw_initialized)
...
int8_t touch_2_poll_u_position(
		uint8_t touch_axis,
		int16_t *u_choord)
{
	if(!is_raw_initialized)
...
int8_t touch_2_poll_s_position(
		uint8_t touch_axis,
		int16_t *s_choord)
{
	if(!is_raw_initialized)
...
int8_t touch_2_wait_raw_position(
		uint8_t touch_axis,
		int16_t *raw_choord)
{
	if(!is_raw_initialized)
...
int8_t touch_2_wait_u_position(
		uint8_t touch_axis,
		int16_t *u_choord)
{
	if(!is_raw_initialized)
...
int8_t touch_2_wait_s_position(
		uint8_t touch_axis,
		int16_t *s_choord)
{
	if(!is_raw_initialized)
