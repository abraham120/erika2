...
int8_t touch_timer_init(uint32_t period, uint32_t fcy) 
{
	...
	pr = period * fcy;
	if (pr <= 0xFFFF) {
		TOUCH_TIMER_REG_PERIOD = pr;
	} else if ((pr / 8) <= 0xFFFF) {
		TOUCH_TIMER_REG_PERIOD = pr / 8;
		TOUCH_TIMER_REG_CONbits.TCKPS = 1;
	} else if ((pr / 64) <= 0xFFFF) {
		TOUCH_TIMER_REG_PERIOD = pr / 64;
		TOUCH_TIMER_REG_CONbits.TCKPS = 2;
	} else if ((pr / 256) <= 0xFFFF) {
		TOUCH_TIMER_REG_PERIOD = pr / 256;
		TOUCH_TIMER_REG_CONbits.TCKPS = 3;
	} else {
		return -2;		/* Unable to have this period! */
	}
	TOUCH_TIMER_INTERRUPT_PRIORITY = 5;
	TOUCH_TIMER_INTERRUPT_FLAG = 0;
	TOUCH_TIMER_REG_TMR = 0;
	return 1;
}
...
COMPILER_ISR(TOUCH_TIMER_INTERRUPT_NAME) 
{
	TOUCH_TIMER_INTERRUPT_FLAG = 0;
