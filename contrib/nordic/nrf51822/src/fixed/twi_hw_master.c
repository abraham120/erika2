 ...
 * the file.
 *
 * Fixed by Pasquale Buonocunto, 2013
 *
 */
...
bool twi_master_transfer(uint8_t address, uint8_t *data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    if (data_length > 0 && twi_master_clear_bus())
    {
		...
		if(NRF_TWI1->EVENTS_ERROR || twi_err_counter > 10)
		{
			twi_reset();
		}
		twi_err_counter += !transfer_succeeded;
...
bool fixed_twi_master_write(uint8_t devAddr, uint8_t address, uint8_t *data, uint8_t data_length)
{
	uint32_t timeout = MAX_TIMEOUT_LOOPS;   /* max loops to wait for EVENTS_TXDSENT event*/
	...
	NRF_TWI1->ADDRESS = (devAddr >> 1);
	
	data_length++;
	
	NRF_TWI1->TXD = address;
	NRF_TWI1->TASKS_STARTTX = 1;
...
bool fixed_twi_master_read(uint8_t devAddr, uint8_t address, uint8_t *data, uint8_t data_length)
{
	return false;
}
