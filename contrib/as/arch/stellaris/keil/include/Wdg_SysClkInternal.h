...
/** @file	Wdg_Internal.h
 *  @brief	AUTOSAR-"like" SysClk WDG (Watchdog) Driver Internal Header
 *  		File.
 *
...
#define VALIDATE(_exp,_api,_err) \
  if( !(_exp) ) { \
    Det_ReportError(WDG_SYSCLK_MODULE_ID,0,_api,_err); \
    return; \
  }
...
#endif	/* ( WDG_SYSCLK_DEV_ERROR_DETECT == STD_ON ) */
...
typedef struct {
  ...
  Wdg_SysClk_StatusType		State;		/* WDG State		      */

  WdgIf_ModeType		Mode;		/* WDG Mode		      */

  boolean			Expired;	/* WDG Timeout Expired	      */

  const Wdg_SysClk_ConfigType *	ConfigPtr;	/* Actual Configuration	      */

}	Wdg_SysClk_GlobalType;

/*
 * Global config
 */
extern	Wdg_SysClk_GlobalType	Wdg_SysClk_Global;
