 ...
 * DMA controller re-arbitrates. The possible arbitration rate configurations
 ...
 *
 *  In this configuration, no arbitration occurs during the DMA transfer
 ...
 *  transfer is less than the arbitration size, the DMA controller uses single
 ...
 *
 * This macros configures the operating mode of the DMA channel cycle.
 */
 ...
 *
 *  When the mode field has this value, the DMA controller does not perform any
 *  transfers and disables the channel if it is enabled.
 *
 *  At the end of a transfer, the DMA controller updates the control word to
 ...
 *
 *  In Basic mode, the DMA controller performs transfers as long as there are
 *  more items to transfer, and a transfer request is present.
 *
 *  This mode is used with peripherals that assert a DMA request signal
 ...
 *
 *  When all of the items have been transferred using Basic mode, the DMA
 ...
 *  received, the transfer runs to completion, even if the DMA request is
 *  removed.
 *
 ...
 *
 *  When all the items have been transferred using Auto mode, the DMA
 *  controller sets the mode for that channel to Stop.
 */
 ...
 *
 *  When the transfer using the primary control structure is complete, the DMA
 ...
 *  contiguous locations in a memory buffer. For example, a gather DMA
 ...
 *  alternate structure where it is then executed. The DMA controller
 ...
 *  performed using Auto mode, the DMA controller stops.
 *
 ...
 *  results in a DMA request.
 *
 *  By programming the DMA controller using this method, a set of arbitrary
 *  transfers can be performed based on a single DMA request.
 */
 ...
 *  except that the transfers are controlled by a peripheral making a DMA
 *  request.
 *
 *  Upon detecting a request from the peripheral, the DMA controller uses the
 ...
 *  a DMA request. The DMA controller continues to perform transfers from the
 *  list only when the peripheral is making a request, until the last transfer
 *  is complete.
 *
 *  A completion interrupt is generated only after the last transfer.
 *
 *  By using this method, the DMA controller can transfer data to or from a
 ...
 *
 * The DMA controller assigns priority to each channel based on the channel
 ...
 *
 * The DMA controller responds to two types of requests from a peripheral:
 * single or burst.
...
/** @brief	Memory Address Low Boundary.	*/
#define	DMA_MEMORY_ADDRESS_LOW		0x20000000U
/** @brief	Memory Address High Boundary.	*/
#define	DMA_MEMORY_ADDRESS_HIGH		0x20008000U

#ifdef	__AS_CFG_DMA_SPI__

/** @brief	SSI 0 Address Low Boundary.	*/
#define	DMA_SSI_0_ADDRESS_LOW		0x40008008U
/** @brief	SSI 0 Address High Boundary.	*/
#define	DMA_SSI_0_ADDRESS_HIGH		DMA_SSI_0_ADDRESS_LOW

/** @brief	SSI 1 Address Low Boundary.	*/
#define	DMA_SSI_1_ADDRESS_LOW		0x40009008U
/** @brief	SSI 1 Address High Boundary.	*/
#define	DMA_SSI_1_ADDRESS_HIGH		DMA_SSI_1_ADDRESS_LOW

/** @brief	SSI 2 Address Low Boundary.	*/
#define	DMA_SSI_2_ADDRESS_LOW		0x4000A008U
/** @brief	SSI 2 Address High Boundary.	*/
#define	DMA_SSI_2_ADDRESS_HIGH		DMA_SSI_2_ADDRESS_LOW

/** @brief	SSI 3 Address Low Boundary.	*/
#define	DMA_SSI_3_ADDRESS_LOW		0x4000B008U
/** @brief	SSI 3 Address High Boundary.	*/
#define	DMA_SSI_3_ADDRESS_HIGH		DMA_SSI_3_ADDRESS_LOW
 ...
 *
 *  Identifier for DMA Driver SPI Configuration.
 */
#define	DMA_CONFIG_SPI		0x00000001U

/** @brief	SPI Configuration Pointer
 *
 *  Pointer of DMA Driver SPI Configuration.
 */
#define	DMA_CONFIG_SPI_PTR	&Dma_Config[DMA_CONFIG_SPI]
