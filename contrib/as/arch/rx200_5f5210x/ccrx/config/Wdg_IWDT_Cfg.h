...
/** @file	Wdg_IWDT_Cfg.h
 *  @brief	fake-druid IWDT AUTOSAR WDG Driver Configuration Header File.
 *
 ...
 *  <tt>Wdg_IWDT_SetTriggerCondition()</tt> is called, the Watchdog timer asserts
 *  its reset signal or the interupt request to the system.
 *
 *  If <tt>Wdg_IWDT_SetTriggerCondition()</tt> is called before the 14-bit
...
#if !(\
 ((WDG_IWDT_SW_MAJOR_VERSION == 1) && \
  (WDG_IWDT_SW_MINOR_VERSION == 0) && \
  (WDG_IWDT_SW_PATCH_VERSION == 0))\
)
#error Wdg_IWDT: Configuration file expected BSW module version to be 1.0.0.*
#endif

#ifndef	WDG_IWDT_CFG_H
#define	WDG_IWDT_CFG_H
 ...
 */
#ifdef	__AS_CFG_WDG_IWDT_DEV_ERROR_DETECT__
#define	WDG_IWDT_DEV_ERROR_DETECT	STD_ON
#else
#define	WDG_IWDT_DEV_ERROR_DETECT	STD_OFF
#endif
 ...
 *  					is not allowed.
 */
#ifdef	__AS_CFG_WDG_IWDT_DISABLE_ALLOWED__
#define	WDG_IWDT_DISABLE_ALLOWED	STD_ON
#else
#define	WDG_IWDT_DISABLE_ALLOWED	STD_OFF
#endif
 ...
 */
#define	WDG_IWDT_INITIAL_TIMEOUT	2.0
 ...
 *  		(Watchdog Counter Max Value / (125000 (Hz)) * Max Clock Div. Ratio
 *  		= 16384 / (125 * 10^3) * 256 = 33.554432 seconds
 */
#define	WDG_IWDT_MAX_TIMEOUT		33.554432
 ...
 *  			- False:	API disabled
 */
#ifdef	__AS_CFG_WDG_IWDT_VERSION_INFO_API__
#define	WDG_IWDT_VERSION_INFO_API	STD_ON
#else
#define	WDG_IWDT_VERSION_INFO_API	STD_OFF
#endif

/** @brief	Watchdog Non-Maskable Interrupt.*/
#define	WDG_IWDT_CTL_NMI_INT	0x00U
/** @brief	Watchdog Reset Enable.*/
#define	WDG_IWDT_CTL_RST_EN		0x80U
/** @brief	Watchdog Count stop function is enabled.*/
#define WDG_IWDT_COUNT_STOP_EN		0x40U
/** @brief	Watchdog Count stop function is disabled.*/
#define WDG_IWDT_COUNT_STOP_DIS		0x00U


#ifdef	__AS_CFG_WDG_IWDT_FAST_MODE_NOTIFICATION__
/** @brief	Watchdog Mode Fast Notification Callback
 *
 *  The notification prototype <tt>Wdg_IWDT_Notification_Fast()</tt> is for
 *  the notification callback function for watchdog mode "fast" and shall be
 *  implemented by the user.
 *
 *  The notifications callback <tt>Wdg_IWDT_Notification_Fast()</tt> shall be
 ...
 *  configured.
 *
 *  When disabled, the IWDT WDG Driver will send no notifications.
 *
 *  The IWDT WDG Driver shall invoke a notification callback whenever the
...
typedef	struct {
	 ...
	 *
	 *  The notification prototype <tt>Wdg_IWDT_Notification()</tt> is for
	 *  the notifications callback function and shall be implemented by the user.
	 *
	 *  The IWDT WDG module's environment shall declare a separate notifications
	 ...
	 */
	Wdg_IWDT_Notification	WdgNotificationPtr;

}	Wdg_IWDT_SettingsFastType;
 ...
 *
 *  The notification prototype <tt>Wdg_IWDT_Notification_Slow()</tt> is for
 *  the notification callback function for watchdog mode "slow" and shall be
 *  implemented by the user.
 *
 *  The notifications callback <tt>Wdg_IWDT_Notification_Slow()</tt> shall be
 *  configurable as pointers to user defined functions within the configuration
 *  structure.
 *
 *  Each watchdog mode shall provide its own notifications callback if
...
#endif	/* __AS_CFG_WDG_IWDT_SLOW_MODE_NOTIFICATION__ */
...
typedef	struct {
	 ...
	 *  Bit/Field	Description
	 *  7	Reset Interrupt Request Selection
	 *  6	Sleep-Mode Count Stop Control Selection
	 *  5:4 Window Start Position Selection
	 *  3:2 Reserved
	 *  1:0 Window End Position Selection
	 */
	 ...
	 *  Function pointer to callback function.
	 *
	 *  The notification prototype <tt>Wdg_IWDT_Notification_<mode>()</tt> is for
	 *  the notifications callback function and shall be implemented by the user.
	 *
	 *  The IWDT WDG module's environment shall declare a separate notifications
	 *  callback for each watchdog mode to avoid parameters in notification
...
typedef struct {
	 ...
	 *  Configuration items for an external watchdog hardware.
	 */
  const Wdg_IWDT_ExternalConfigType *	WdgExternalConfiguration;
#endif
 ...
 *  Hardware dependent settings for the watchdog driver's "fast" mode.
 */
  const Wdg_IWDT_SettingsFastType *	WdgSettingsFast;
   ...
   *  Hardware dependent settings for the watchdog driver's "off" mode.
   */
  const Wdg_IWDT_SettingsOffType *	WdgSettingsOff;
#endif	/* ( WDG_IWDT_DISABLE_ALLOWED == STD_ON ) */
   ...
   *  Hardware dependent settings for the watchdog driver's "slow" mode.
   */
  const Wdg_IWDT_SettingsSlowType *	WdgSettingsSlow;

}	Wdg_IWDT_SettingsConfigType;

/** @brief	IWDT WDG Module Configuration Parameters
 *
 ...
 *  			(<tt>BSW00414</tt>)
 */
typedef	Wdg_IWDT_SettingsConfigType	Wdg_IWDT_ConfigType;
 ...
 */
#define	WDG_IWDT_TOGGLE	0x00000000U
#define	WDG_IWDT_WINDOW	0x00000001U
#define	WDG_IWDT_BOTH	0x00000002U
 ...
 */
#define	WDG_IWDT_TRIGGER_MODE	WDG_IWDT_WINDOW;
#endif	/* 0 */

/** @brief	Test Configuration
 *
 *  Identifier for Test IWDT WDG Driver Configuration.
 */
#define	WDG_IWDT_CONFIG_TEST		0x00000000U

/** @brief	Test Configuration Pointer
 *
 *  Pointer of Test WDG Driver Configuration.
 */
#define	WDG_IWDT_CONFIG_TEST_PTR	\
	&Wdg_IWDT_Config[WDG_IWDT_CONFIG_TEST]

/** @brief	Default Configuration
 *
 *  Identifier for Default IWDT WDG Driver Configuration.
 */
#define	WDG_IWDT_CONFIG_DEFAULT	WDG_IWDT_CONFIG_TEST

/** @brief	Default Configuration Pointer
 *
 *  Pointer of Default IWDT WDG Driver Configuration.
 */
#define	WDG_IWDT_CONFIG_DEFAULT_PTR	\
	&Wdg_IWDT_Config[WDG_IWDT_CONFIG_DEFAULT]
...
#if defined (__AS_CFG_WDG_IWDT_SLOW_MODE_RESET__) || \
		defined(__AS_CFG_WDG_IWDT_SLOW_MODE_NMI__)

#define WDGIF_DEFAULT_MODE WDGIF_SLOW_MODE

#elif defined (__AS_CFG_WDG_IWDT_FAST_MODE_RESET__) || \
		defined(__AS_CFG_WDG_IWDT_FAST_MODE_NMI__)

#define WDGIF_DEFAULT_MODE WDGIF_FAST_MODE

#else

#error "Either (__AS_CFG_WDG_IWDT_SLOW_MODE_RESET__ or\
 __AS_CFG_WDG_IWDT_SLOW_MODE_NMI__  ) or\
 (__AS_CFG_WDG_IWDT_FAST_MODE_RESET__ or\
 __AS_CFG_WDG_IWDT_FAST_MODE_NMI__  ) should be defined!"
#endif

#endif	/* WDG_IWDT_CFG_H */
