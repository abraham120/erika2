...
/*
 * Usefull IO Port Macros
 */

#define	IO_PORT_DATA_ADDRESS_MASK	0x0000001F

#define	IO_PORT_ADDR(_port)	(uint32)( \
  (uint32)HW_IO_BASE_ADDR + \
  ((uint32)_port  & (uint32)IO_PORT_DATA_ADDRESS_MASK) \
)
...
Dio_LevelType Dio_ReadChannel (Dio_ChannelType	ChannelId)
{
	...
	VALIDATE_W_RV( ( ChannelId < DIO_CHANNELS_NUMBER ), DIO_READCHANNEL_ID, 
			DIO_E_PARAM_INVALID_CHANNEL_ID, STD_LOW);

	f = EE_hal_suspendIRQ();

	VALIDATE_IRQ_W_RV(Dio_Global.Init, DIO_READCHANNEL_ID, DIO_E_PARAM_CONFIG, 
			STD_LOW, f);

	/* Port Look-up. */
	c = HW_CH_2_MOD(ChannelId);
	for (p = 0;(( p < Dio_Global.ConfigPtr->DioNumberOfPorts ) && 
			( c != Dio_Global.ConfigPtr->DioPorts[p].DioPortId )); p++);

	VALIDATE_IRQ_W_RV( ( p < Dio_Global.ConfigPtr->DioNumberOfPorts ), 
			DIO_READCHANNEL_ID, DIO_E_PARAM_INVALID_CHANNEL_ID, STD_LOW, f);

	/* Channel Look-up. */
	for ( c = 0; (( c < Dio_Global.ConfigPtr->DioPorts[p].DioNumberOfChannels ) 
...
void Dio_WriteChannel (Dio_ChannelType	ChannelId, Dio_LevelType Level)
{
	...
	VALIDATE(( ChannelId < DIO_CHANNELS_NUMBER ), DIO_WRITECHANNEL_ID, 
	...
	VALIDATE_IRQ( ( p < Dio_Global.ConfigPtr->DioNumberOfPorts ), 
			DIO_WRITECHANNEL_ID, DIO_E_PARAM_INVALID_CHANNEL_ID, f);
	...
	VALIDATE_IRQ(( c < Dio_Global.ConfigPtr->DioPorts[p].DioNumberOfChannels ), 
			DIO_WRITECHANNEL_ID, DIO_E_PARAM_INVALID_CHANNEL_ID, f);

	IO_SET_PIN(ChannelId, Level);
...
Dio_PortLevelType Dio_ReadPort (Dio_PortType PortId)
{
	...
	VALIDATE_W_RV( ( PortId < DIO_PORTS_NUMBER ), DIO_READPORT_ID, 
			DIO_E_PARAM_INVALID_PORT_ID, STD_LOW);

	f = EE_hal_suspendIRQ();

	VALIDATE_IRQ_W_RV( Dio_Global.Init, DIO_READPORT_ID, DIO_E_PARAM_CONFIG, 
			STD_LOW, f);

	/* Port Look-up. */
	for ( p = 0; (( p < Dio_Global.ConfigPtr->DioNumberOfPorts ) &&
			( PortId != Dio_Global.ConfigPtr->DioPorts[p].DioPortId )); p++);

	VALIDATE_IRQ_W_RV(( p < Dio_Global.ConfigPtr->DioNumberOfPorts ), 
			DIO_READPORT_ID, DIO_E_PARAM_INVALID_PORT_ID, STD_LOW, f);

	lvl = IO_GET_PORT(PortId);
...
void Dio_WritePort (Dio_PortType PortId, Dio_PortLevelType Level)
{
	...
	VALIDATE( ( PortId < DIO_PORTS_NUMBER ), DIO_WRITEPORT_ID, 
	...
	VALIDATE_IRQ( ( p < Dio_Global.ConfigPtr->DioNumberOfPorts ), 
			DIO_WRITEPORT_ID, DIO_E_PARAM_INVALID_PORT_ID, f);

	IO_SET_PORT(PortId, Level);
								...
								const Dio_ChannelGroupType*	ChannelGroupIdPtr)
{
	...
	VALIDATE_W_RV( ( ChannelGroupIdPtr != NULL_PTR ), DIO_READCHANNELGROUP_ID, 
			DIO_E_PARAM_POINTER, STD_LOW);

	VALIDATE_W_RV( ( ChannelGroupIdPtr->port < DIO_PORTS_NUMBER ), 
			DIO_READCHANNELGROUP_ID, DIO_E_PARAM_INVALID_PORT_ID, STD_LOW);

	f = EE_hal_suspendIRQ();

	VALIDATE_IRQ_W_RV( Dio_Global.Init, DIO_READCHANNELGROUP_ID, 
			DIO_E_PARAM_CONFIG, STD_LOW, f);
	...
	VALIDATE_IRQ_W_RV(( p < Dio_Global.ConfigPtr->DioNumberOfPorts ), 
			DIO_READCHANNELGROUP_ID, DIO_E_PARAM_INVALID_PORT_ID, STD_LOW, f);

	lvl = (( IO_GET_PORT(ChannelGroupIdPtr->port) & ChannelGroupIdPtr->mask ) >>
							...
							Dio_PortLevelType	Level)
{
	...
	VALIDATE(( ChannelGroupIdPtr != NULL_PTR ), DIO_WRITECHANNELGROUP_ID,	
			DIO_E_PARAM_POINTER);

	VALIDATE( ( ChannelGroupIdPtr->port < DIO_PORTS_NUMBER ), 
			DIO_WRITECHANNELGROUP_ID, DIO_E_PARAM_INVALID_PORT_ID);

	f = EE_hal_suspendIRQ();

	VALIDATE_IRQ( Dio_Global.Init, DIO_WRITECHANNELGROUP_ID, DIO_E_PARAM_CONFIG, 
			f);
	...
	VALIDATE_IRQ(( p < Dio_Global.ConfigPtr->DioNumberOfPorts ), 
			DIO_WRITECHANNELGROUP_ID, DIO_E_PARAM_INVALID_PORT_ID, f);

	IO_SET_PORT( ChannelGroupIdPtr->port, ((Level << ChannelGroupIdPtr->offset) 
...
Dio_LevelType Dio_FlipChannel (Dio_ChannelType	ChannelId)
{
	...
	VALIDATE_W_RV( ( ChannelId < DIO_CHANNELS_NUMBER ), DIO_FLIPCHANNEL_ID,
				DIO_E_PARAM_INVALID_CHANNEL_ID, STD_LOW);

	f = EE_hal_suspendIRQ();

	VALIDATE_IRQ_W_RV( Dio_Global.Init, DIO_FLIPCHANNEL_ID, DIO_E_PARAM_CONFIG, 
			STD_LOW, f);
	...
	VALIDATE_IRQ_W_RV(( p < Dio_Global.ConfigPtr->DioNumberOfPorts ), 
			DIO_FLIPCHANNEL_ID, DIO_E_PARAM_INVALID_CHANNEL_ID, STD_LOW, f);
	...
	if ( lvl == STD_LOW ) {
		IO_SET_PIN(ChannelId, STD_HIGH);
	} else {
		IO_SET_PIN(ChannelId, STD_LOW);
	}
