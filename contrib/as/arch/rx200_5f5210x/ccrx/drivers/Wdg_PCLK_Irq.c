...
/*
 * Wdg_PCLK_Irq.c File for implementation of interrupt service routine
 *
...
/*
 * Wdg_PCLK_Irq.c has implicit access to the Wdg_PCLK_Cfg.h through the
 * Wdg_PCLK.h file.
 */
#include "Wdg_PCLK.h"
...
static void Wdg_PCLK_Isr(void) {
	...
	if ( Wdg_PCLK_Global.State == WDG_IDLE &&  
		...
		if ( Wdg_PCLK_Global.Expired == FALSE ) {
			...
			Wdg_PCLK_Global.Expired = TRUE;

			if ( Wdg_PCLK_Global.Mode == WDGIF_FAST_MODE ) {

				if (( Wdg_PCLK_Global.ConfigPtr->WdgSettingsFast != NULL_PTR ) 
						&& ( Wdg_PCLK_Global.ConfigPtr->WdgSettingsFast->WdgNotificationPtr != NULL_PTR)) {

					EE_hal_resumeIRQ(flags);

					(*(Wdg_PCLK_Global.ConfigPtr->WdgSettingsFast->WdgNotificationPtr))();
				...
				if (( Wdg_PCLK_Global.ConfigPtr->WdgSettingsSlow != NULL_PTR ) &&
						( Wdg_PCLK_Global.ConfigPtr->WdgSettingsSlow->WdgNotificationPtr != NULL_PTR)) {
					
					EE_hal_resumeIRQ(flags);
					(*(Wdg_PCLK_Global.ConfigPtr->WdgSettingsSlow->WdgNotificationPtr))();
	...
	}	/* if ( Wdg_PCLK_Global.State == WDG_IDLE ) && ( Wdg_PCLK_Global.Mode != WDGIF_OFF_MODE ) */
