/**************************************************************************//**
 * $Id: lpc12xx_comp.c 7535 2011-06-09 09:13:16Z cnh20509 $
 *
 * @file     lpc12xx_comp.c
...
void COMP_Cmd(uint32_t COMP_INDEX, FunctionalState NewState)
{
	...
	if (NewState == ENABLE)
	{
	    if (COMP_INDEX == COMP_INDEX_0)
	        LPC_ACOMP->CMP |= COMP0_ENABLE;  /* Enable comparator 0 */
	    else
	        LPC_ACOMP->CMP |= COMP1_ENABLE;	/* Enable comparator 1 */       
	}
	else
	{
	    if (COMP_INDEX == COMP_INDEX_0)
	        LPC_ACOMP->CMP &= COMP0_DISABLE; /* Disable comparator 0 */
	    else
	        LPC_ACOMP->CMP &= COMP1_DISABLE;	/* Disable comparator 1 */    
	}
}
...
void COMP_IntConfig (uint32_t COMP_INTSRC)
{
    ...
    LPC_ACOMP->CMP &= ~CMP_INTSRC_MASK;
    
    /* Config interrupt source */
    LPC_ACOMP->CMP |= COMP_INTSRC;      
}
...
FlagStatus COMP_GetOutputStatus (uint32_t COMP_INDEX) 
{
    ...
    if (COMP_INDEX == COMP_INDEX_0)
	    return (((LPC_ACOMP->CMP & CMP_STAT0_MASK) == (uint32_t)RESET) ? RESET : SET);
	else
	    return (((LPC_ACOMP->CMP & CMP_STAT1_MASK) == (uint32_t)RESET) ? RESET : SET);
...
void COMP0_InputConfig (uint32_t COMP_INPORT, uint32_t COMP0_INCHANNEL)
{
    ...
    {
        LPC_ACOMP->CMP |= (COMP0_INCHANNEL << BIT_CMP_CMP0_VP_CTRL) & CMP_CMP0_VP_CTRL_MASK;
    }
    else
    {
        LPC_ACOMP->CMP |= (COMP0_INCHANNEL << BIT_CMP_CMP0_VM_CTRL) & CMP_CMP0_VM_CTRL_MASK;
    }
...
void COMP1_InputConfig (uint32_t COMP_INPORT, uint32_t COMP1_INCHANNEL)
{
    ...
    if (COMP_INPORT == COMP_INPORT_POSITIVE)
    {
        LPC_ACOMP->CMP |= (COMP1_INCHANNEL << BIT_CMP_CMP1_VP_CTRL) & CMP_CMP1_VP_CTRL_MASK;
    }
    else
    {
        LPC_ACOMP->CMP |= (COMP1_INCHANNEL << BIT_CMP_CMP1_VM_CTRL) & CMP_CMP1_VM_CTRL_MASK;
    }
...
void COMP_VolLadderCmd (FunctionalState NewState)
{
    ...
    if (NewState == ENABLE)
    {
        LPC_ACOMP->VLAD |= VLAD_ENABLE;   /* Enalbes voltage ladder */
    }
    else
    {
        LPC_ACOMP->VLAD &= VLAD_DISABLE;  /* Disables voltage ladder */
    }
}
...
void COMP_VolLadderConfig (uint8_t COMP_VLADREF, uint8_t MValue)
{
    ...
    LPC_ACOMP->VLAD &= ~VLAD_VOLTAGE_MASK;
    
    LPC_ACOMP->VLAD |= (COMP_VLADREF) | ((MValue&0x1F) << BIT_VLAD_VSEL);        
}
