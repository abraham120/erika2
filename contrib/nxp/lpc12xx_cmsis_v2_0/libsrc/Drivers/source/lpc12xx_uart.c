/**************************************************************************//**
 * $Id: lpc12xx_uart.c 7535 2011-06-09 09:13:16Z cnh20509 $ 
 *
 * @file     lpc12xx_uart.c
...
static Status uart_set_divisors(LPC_UART_TypeDef *UARTx, uint32_t baudrate)
{
	...
	/* We did not differeniate UART0 and UART1 here since both have the 
	same offset and name for below registers.  The same for other functions. */
...
void UART_Init(LPC_UART_TypeDef *UARTx)
{
    ...
    if ((LPC_UART0_TypeDef *)UARTx == LPC_UART0){   
        ...
        ((LPC_UART1_TypeDef *)UARTx)->ICR = 0;         // Set IrDA to default state
    }
#endif
...
/*
  ...
  * @return none
  */
void UART_SetConfigModem(LPC_UART0_TypeDef *UARTx, UART_MODEM_CFG_Type *config)
{
    uint8_t tmp;
...
void UART_GetConfigModem(LPC_UART0_TypeDef *UARTx, UART_MODEM_CFG_Type *config)
{
    CHECK_PARAM(PARAM_UART_MODEM(UARTx));
...
/*
  ...
  * @return  None
  */
void UART_SetConfigRS485(LPC_UART0_TypeDef *UARTx, UART_RS485_CFG_Type *config)
{
    uint32_t tmp;
...
/**
  ...
  * @return  None
  */
void UART_GetConfigRS485(LPC_UART0_TypeDef *UARTx, UART_RS485_CFG_Type *config)
{
    CHECK_PARAM(PARAM_UART_485(UARTx));
...
/*
  ...
  * @return  None
  */
void UART_SetConfigIRDA(LPC_UART1_TypeDef *UARTx, UART_IRDA_CFG_Type *config)
{
    volatile uint32_t tmp;
...
/**
  ...
  * @return  None
  */
void UART_GetConfigIRDA(LPC_UART1_TypeDef *UARTx, UART_IRDA_CFG_Type *config)
{
    CHECK_PARAM(PARAM_UART_IRDA(UARTx));
