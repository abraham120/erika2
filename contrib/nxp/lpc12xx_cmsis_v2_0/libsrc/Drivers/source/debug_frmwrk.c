/**************************************************************************//**
 * $Id: debug_frmwrk.c 7535 2011-06-09 09:13:16Z cnh20509 $
 *
 * @file      debug_frmwrk.c
...
static void db_putchar (uint8_t ch)
{
    UART_Send((LPC_UART_TypeDef*)DEBUG_UART_PORT, &ch, 1, UART_BLOKING_TIMEOUT);
    while (!(UART_GetLineStatus((LPC_UART_TypeDef*)DEBUG_UART_PORT) & UART_LS_TX_EMPTY_ALL));
}
...
void DB_Init(void)
{
    ...
    IOCON_SetFunc(&PIO_mode);    
    if((LPC_UART0_TypeDef*)DEBUG_UART_PORT == LPC_UART0){ 
    ...
    UART_Init((LPC_UART_TypeDef*)DEBUG_UART_PORT);

    UART_GetConfig((LPC_UART_TypeDef*)DEBUG_UART_PORT, &UARTConfigStruct);
    UARTConfigStruct.baudrate = 9600;   //115200;
    UARTConfigStruct.databits = UART_CFG_DATABIT_8;

    // Initialize UART0 peripheral with given to corresponding parameter
    UART_SetConfig((LPC_UART_TypeDef*)DEBUG_UART_PORT, &UARTConfigStruct);
    UART_ConfigTXD((LPC_UART_TypeDef*)DEBUG_UART_PORT, ENABLE);
}
...
uint8_t DB_GetChar (void)
{
    ...
    while(len==0)
    {
        len = UART_Receive((LPC_UART_TypeDef*)DEBUG_UART_PORT, &tmp, 1, UART_BLOKING_TIMEOUT);
    }
