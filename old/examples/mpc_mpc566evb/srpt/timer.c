 ...
 * CVS: $Id: timer.c,v 1.3 2004/11/30 14:52:03 pj Exp $
 */
...
extern const ERIKA_TYPERELDLINE th_period[];
static ERIKA_TIME time_countdown[THREAD_MAX];

/* elapsed time since last ticks handler activation */
static ERIKA_TIME last_handler_activation;
...
{
    int i;
    ERIKA_TIME min_dline;
    
    /* timer activation */
    last_handler_activation = ERIKA_timer_0_get();
	...
	ERIKA_thread_activate(i);
	if (th_period[i] < min_dline){
	    min_dline = time_countdown[i];
	}
    }

    ERIKA_timer_1_set(min_dline);
    ERIKA_timer_1_start();
...
void tick_handler(void)
{
    ...
    ERIKA_TIME min_dline;
    ERIKA_TIME this_handler_activation;
    ERIKA_TIME elapsed_time;
    
    /* calculate the last activation time */
    this_handler_activation = ERIKA_timer_0_get();
