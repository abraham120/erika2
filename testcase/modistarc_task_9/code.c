...
TASK(Task1)
{
  ...
  s = GetTaskID(&TaskID);
  EE_assert(2, (s==E_OK && TaskID==Task1), 1);
#else
  GetTaskID(&TaskID);
  EE_assert(2, (TaskID==Task1), 1);
#endif

#ifdef __EXTENDED_STATUS__
  s = GetTaskState(Task1, &TaskState);
  EE_assert(3, (s==E_OK && TaskState==RUNNING), 2);
#else
  GetTaskState(Task1, &TaskState);
  EE_assert(3, (TaskState==RUNNING), 2);
#endif

#ifdef __EXTENDED_STATUS__
  s = GetTaskState(Task2, &TaskState);
  EE_assert(4, (s==E_OK && TaskState==SUSPENDED), 3);
#else
  GetTaskState(Task2, &TaskState);
  EE_assert(4, (TaskState==SUSPENDED), 3);
#endif
