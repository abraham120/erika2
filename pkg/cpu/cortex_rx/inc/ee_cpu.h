...
#define CORTEX_RX_INIT_TOS_OFFSET 10
#endif
...
extern EE_UINT32  __current_sp(void);
#ifdef __GNU__
...
#ifdef __CORTEX_RX__
#include "cpu/cortex_rx/inc/ee_systick.h"
#endif
...
//#define EE_cortex_rx_are_IRQs_enabled(ie) ((ie) ^ 1)


/**
 * Return true (not 0) if IRQs are enabled, 0 (false) if IRQ are disabled.
 */
__INLINE__ EE_UINT32  EE_cortex_rx_get_IRQ_enabled(void)
{
  return ((_get_CPSR() & 0x80) == 0);
 ...
 * Enable interrupts
 */
__INLINE__ void  EE_cortex_rx_enableIRQ(void)
{
 ...
 * Disable interrupts
 */
__INLINE__ void  EE_cortex_rx_disableIRQ(void)
{
#ifdef __IAR__
	__ASM ("cpsid i");
#else	/* __IAR__ */
 ...
 * Resume interrupts
 */
__INLINE__ void  EE_cortex_rx_resumeIRQ(EE_FREG state)
{
  if (state){
 ...
 * Suspend interrupts
 */
__INLINE__ EE_FREG  EE_cortex_rx_suspendIRQ(void)
{
  EE_FREG istat;
  istat = EE_cortex_rx_get_IRQ_enabled();
  EE_cortex_rx_disableIRQ();
  return istat;
...
__INLINE__ EE_TYPEISR2PRIO  EE_cortex_rx_get_int_prio(void)
{
  return 0;
...
__INLINE__ void  EE_cortex_rx_set_int_prio(
...
__INLINE__ EE_TYPEISR2PRIO  EE_cortex_rx_get_isr_prio(
  void
)
{
 return 0;
// #ifdef __CORTEX_R4__
  ...
  // if (vectact > EE_CORTEX_RX_SYSTICK_EXC_NUM) {
    // vectact -= EE_CORTEX_RX_SYSTICK_EXC_NUM + 1;
    // prio = NVIC_GET_PRI(vectact);
  // }
// #ifdef	__USE_SYSTICK__
  // else if (vectact == EE_CORTEX_RX_SYSTICK_EXC_NUM) {
    // prio = NVIC_SYS_PRI3_R >> NVIC_SYS_PRI3_TICK_S;
  // }
// #endif
// #ifdef	__USE_SVC__
  // else if (vectact == EE_CORTEX_RX_SVCALL_EXC_NUM) {
