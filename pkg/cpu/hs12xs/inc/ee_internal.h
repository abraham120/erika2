...
#if !defined(RTDRUID_CONFIGURATOR_NUMBER) || (RTDRUID_CONFIGURATOR_NUMBER<1276)
    void EE_s12_terminate_savestk(EE_DADD sp, EE_ADDR realbody);
    void EE_s12_terminate_task(EE_DADD sp);
#else
    void EE_s12_terminate_savestk(EE_ADDR sp, EE_FADDR realbody);
    void EE_s12_terminate_task(EE_ADDR sp);
#endif

__INLINE__ void __ALWAYS_INLINE__ EE_hal_terminate_savestk(EE_TID t) {
    EE_s12_terminate_savestk( &EE_terminate_data[t], 
                              (EE_FADDR)EE_terminate_real_th_body[t]);
}

__INLINE__ void __ALWAYS_INLINE__ EE_hal_terminate_task(EE_TID t) {
  EE_s12_terminate_task(&EE_terminate_data[t]);
}

#endif

#endif	/* __INCLUDE_HC12_INTERNAL_H__ */
