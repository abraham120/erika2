...
CG_OBJDUMP_EXTENSION := objdump

#IAR_CCDIR refers to the location of IAR libraries
BINDIR_CYG := /usr/bin
BINDIR_IAR := $(call short_native_path, $(COMP_DIR)/bin)

CG_LIB_DIR := $(call short_native_path, $(COMP_DIR)/lib)

CG_INCLUDE_DIR := $(call short_native_path, $(COMP_DIR)/inc/)
...
ifndef EE_ASM
EE_ASM:=$(BINDIR_ASM)/iasmrh850.exe
endif

ifndef EE_CC
EE_CC:=$(BINDIR_CC)/iccrh850.exe
endif

ifndef EE_LINK
EE_LINK:=$(BINDIR_ASM)/ilinkrh850.exe
endif

ifndef EE_DEP
EE_DEP:=$(BINDIR_DEP)/iccrh850.exe
endif
...
#Note: -e is to use language extentions

OPT_CC = -O -e --silent 
ifeq ($(call iseeopt, DEBUG), yes)
OPT_CC += --debug --dlib_config $(CG_LIB_DIR)/DLib_Config_Normal.h
endif
...
OPT_LINK += --silent --config_def CSTACK_SIZE=0x1000 --config_def HEAP_SIZE=4096
...
ASM_EEOPT := $(subst __RH850__, , $(EEOPT))
DEFS_ASM = $(addprefix -D, $(ASM_EEOPT) )
