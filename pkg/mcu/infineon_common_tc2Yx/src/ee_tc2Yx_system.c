...
static void EE_tc2Yx_stm_set_sr0(EE_UINT32 usec, EE_TYPEISR2PRIO intvec);
static void EE_tc2Yx_stm_set_sr0_next_match(EE_UINT32 usec);
#else
...
static void EE_tc2Yx_stm_set_sr1(EE_UINT32 usec, EE_TYPEISR2PRIO intvec);
static void EE_tc2Yx_stm_set_sr1_next_match(EE_UINT32 usec);
#else
...
#if (EE_SYSTEM_TIMER_DEVICE == EE_TC_STM_SR0)
  EE_tc2Yx_stm_set_sr0_next_match(OSTICKDURATION / 1000U);
...
#if (EE_SYSTEM_TIMER_DEVICE == EE_TC_STM_SR0)
  EE_tc2Yx_stm_set_sr0(OSTICKDURATION / 1000U,
    (EE_TYPEISR2PRIO)EE_ISR2_ID_EE_tc_system_timer_handler);
#elif (EE_SYSTEM_TIMER_DEVICE == EE_TC_STM_SR1)
  EE_tc2Yx_stm_set_sr1(OSTICKDURATION / 1000U,
    (EE_TYPEISR2PRIO)EE_ISR2_ID_EE_tc_system_timer_handler);
#endif
}
...
static EE_UINT32 EE_tc2Yx_stm_freq_khz;

static EE_UINT32 EE_tc2Yx_stm_us_ticks ( EE_UINT32 usec ) {
  EE_UINT32 ticks;
  if ( EE_tc2Yx_stm_freq_khz >= EE_KILO ) {
    ticks = usec * ( EE_tc2Yx_stm_freq_khz / EE_KILO );
  } else if ( usec >= EE_KILO ) {
    ticks = ( usec / EE_KILO ) * EE_tc2Yx_stm_freq_khz;
  } else {
    ticks = ( usec * EE_tc2Yx_stm_freq_khz ) / EE_KILO;
  }
  return ticks;
    ...
    EE_TYPEISR2PRIO intvec)
{
  ...
  if ( intvec != (EE_TYPEISR2PRIO)EE_ISR_UNMASKED ) {
     ...
     */
    EE_STM_SR0.U =  EE_TC2YX_SRN_TYPE_OF_SERVICE(EE_CURRENTCPU) |
  ...
  EE_TYPEISR2PRIO intvec)
{
  ...
  if ( intvec != (EE_TYPEISR2PRIO)EE_ISR_UNMASKED ) {
     ...
     *  [11..12] Type Of Service (means which CPU will handle it)
     */
    EE_STM_SR1.U = EE_TC2YX_SRN_TYPE_OF_SERVICE(EE_CURRENTCPU) |
      EE_TC2YX_SRN_ENABLE | EE_TC2YX_SRN_PRIORITY(intvec);
  } else {
    /* Disable STM Service Request Source */
...
{
  /* Read Start Point */
  EE_UREG const start = EE_tc2Yx_stm_get_time_lower_word();
  /* Evaluate End Point */
  EE_UREG const ticks = EE_tc2Yx_stm_us_ticks(usec);

  while ( ticks > EE_tc2Yx_stm_get_time_lower_word() - start ) {
    ; /* Wait */
  }
}
