 ...
 ******************************************************************************/
#ifndef INCLUDE_EE_TC27X_MULTICORE_H__
#define INCLUDE_EE_TC27X_MULTICORE_H__
...
#define EE_CPU1_START_ADDR  EE_tc27x_cpu1_start
#endif  /* EE_CPU1_START_ADDR */

#ifndef EE_CPU2_START_ADDR
#define EE_CPU2_START_ADDR  EE_tc27x_cpu2_start
 ...
 */
__INLINE__ void __ALWAYS_INLINE__ EE_tc27x_setup_inter_irqs( void )
{
  ...
  SRC_GPSR00.U = EE_TC27X_SRN_TYPE_OF_SERVICE(0U) | EE_TC27X_SRN_ENABLE |
    EE_TC27X_SRN_PRIORITY(EE_ISR_PRI_1);
  SRC_GPSR10.U = EE_TC27X_SRN_TYPE_OF_SERVICE(1U) | EE_TC27X_SRN_ENABLE |
    EE_TC27X_SRN_PRIORITY(EE_ISR_PRI_1);
  SRC_GPSR20.U = EE_TC27X_SRN_TYPE_OF_SERVICE(2U) | EE_TC27X_SRN_ENABLE |
    EE_TC27X_SRN_PRIORITY(EE_ISR_PRI_1);
}
...
/* Signal the core "cpu" (0/1/2) by sending an IIRQ. This API Must handle more
   ...
   macro). */
__INLINE__ void __ALWAYS_INLINE__ EE_tc27x_signal_cpu( EE_TYPECOREID cpu )
{
  if ( cpu == 0U ) {
    SRC_GPSR00.U |= EE_TC27X_SRN_SET_REQUEST;
  } else if ( cpu == 1U ) {
    SRC_GPSR10.U |= EE_TC27X_SRN_SET_REQUEST;
  } else if ( cpu == 2U ) {
    SRC_GPSR20.U |= EE_TC27X_SRN_SET_REQUEST;
  } else {
...
__INLINE__ void __ALWAYS_INLINE__ EE_tc27x_ack_signal( EE_TYPECOREID cpu )
{
  if ( cpu == 0U ) {
    SRC_GPSR00.U |= EE_TC27X_SRN_CLEAR_REQUEST;
  } else if ( cpu == 1U ) {
    SRC_GPSR10.U |= EE_TC27X_SRN_CLEAR_REQUEST;
  } else if ( cpu == 2U ) {
    SRC_GPSR20.U |= EE_TC27X_SRN_CLEAR_REQUEST;
  } else {
    /* Empty else statement to comply with MISRA 14.10 */
  }
}

#define EE_tc_ack_signal(cpu)   EE_tc27x_ack_signal(cpu)
#define EE_tc_signal_cpu(cpu)   EE_tc27x_signal_cpu(cpu)
...
#define EE_TC27X_START    EE_tc27x_cpu1_start /* Core Start-up code entry */
...
#define EE_TC27X_START    EE_tc27x_cpu2_start /* Core Start-up code entry */
