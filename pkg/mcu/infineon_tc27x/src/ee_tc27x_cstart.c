...
#define EE_tc27x_extru2(src,start,size) \
  ({ int res; __asm volatile (" extr.u\t %0,%1,%2,%3" : "=d" (res) : "d" (src),\
  "i" (start),"i" (size) : "memory"); res; })
#define EE_tc27x_extru(src,start,size) EE_tc27x_extru2(src,start,size)
...
static void EE_tc27x_cinit( void );
...
extern EE_FAR void EE_INT_TAB ( void );   /* interrupt table */
extern EE_FAR void EE_TRAP_TAB( void );   /* trap table */
...
__INLINE__ void __ALWAYS_INLINE__ EE_tc27x_csa_init( void );
...
/***************** Configure PLL *****************/
  ...
  /* The internal version of EE_tc27x_configure_clock is an inline
     ...
     (thing done later) */
  EE_tc27x_configure_clock_internal(EE_CPU_CLOCK);
   ...
   */
  EE_tc27x_setareg(a0, EE_SMALL_DATA);

  EE_tc27x_setareg(a1, EE_LITERAL_DATA);
#ifdef __TASKING__
  ...
  EE_tc27x_setareg(a8, EE_A8_DATA);

  EE_tc27x_setareg(a9, EE_A9_DATA);
#endif /* __TASKING__ */
   ...
   */
  EE_tc27x_cinit();      /* initialize data */
