...
void EE_Gpio_Channel_A_7_CallBack(void)
{
	...
	EE_Gpio_FlipChannel( EE_GPIO_CHANNEL_C_7, &Value);
}
...
int main(void) {
	...
	while ( clkstat != EE_MCU_CLK_STABLE );

	EE_Mcu_DistributeClock(
	  EE_MCU_CLK_MUX_FLAG_RC32MHZ,
	  EE_MCU_CLK_DIV_FLAG_DEFAULT
	);
