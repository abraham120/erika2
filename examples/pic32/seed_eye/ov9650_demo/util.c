...
void myprintf(const char *format, ...)
{
	...
	console_write(MY_DEBUG_CONSOLE_TX, str, MAXCHARS - 1 );
	console_write(MY_DEBUG_CONSOLE_TX, printf_trunc, sizeof(printf_trunc) - 1);

    } else {
        console_write(MY_DEBUG_CONSOLE_TX, str, len);
    }
    EE_pic32_enableIRQ();
}
...
void hex_dump(const void *base, int size)
{
    ...
    for (k = 0; k < size; ++k) {
        ...
        if (0 == ((k+1) % 16) || k == size-1) {
            lbuf[j++] = '\n';
            lbuf[j] = 0;
            console_write(MY_DEBUG_CONSOLE_TX, lbuf, j);
            j = 0;
        } else if (0 == ((k+1) % 8)) {
            lbuf[j++] = ' ';
        }
