...
volatile unsigned char my_button_status_1 = 0; 
unsigned char kitt_i = 0;
unsigned char kitt_seqs[14] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 
								0x40, 0x20, 0x10, 0x08, 0x04, 0x02};

TASK(TaskKitt)
{
	/* Set LEDs new status */
	if(!my_button_status_1){
		EE_led_set_all(kitt_seqs[kitt_i]);
		kitt_i = ( (kitt_i+1) % (sizeof(kitt_seqs)/sizeof(kitt_seqs[0])));
	} 
}
...
int main(void)
{
	...
	SetRelAlarm(AlarmKitt,  100, 100);
	SetRelAlarm(AlarmFlash, 1,  1);
	EE_timer_on();
