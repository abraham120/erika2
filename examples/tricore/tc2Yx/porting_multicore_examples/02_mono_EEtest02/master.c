...
/*
 * Interrupt inter core communication
 */
ISR(ipc_interrupt)
{
  EE_tc2Yx_ack_signal(0);
  timer_h();
}
...
ISR(timer_interrupt)
{
  EE_tc2Yx_stm_set_sr1_next_match(1000);
  EE_tc2Yx_signal_cpu(0);
  EE_tc2Yx_signal_cpu(1);
}

/*
 * MAIN TASK
 */
EE_TYPEBARRIER EE_SHARED_UDATA start_barrier;
...
int main(void)
{
  ...
  SRC_GPSR00.U = EE_TC2YX_SRN_TYPE_OF_SERVICE(0U) | EE_TC2YX_SRN_ENABLE |
    EE_TC2YX_SRN_PRIORITY(EE_ISR2_ID_ipc_interrupt);

  EE_tc2Yx_stm_set_clockpersec( );
  EE_tc2Yx_stm_set_sr1(1000, EE_ISR2_ID_timer_interrupt);
  EE_tc_enableIRQ();
  started = 1;
