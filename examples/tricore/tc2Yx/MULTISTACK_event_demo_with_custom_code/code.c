...
/* Declare system Timer handler: TO BE added to default interrupt vecrtor */
extern void EE_tc_system_timer_handler ( void );
...
int main( void )
{
  ...
  /* I should not do this for sure... */
  //EE_tc_endint_disable();
  //__mtcr(BTV, (EE_UINT32)TriCore_trap_table);
  //__mtcr(BIV, (EE_UINT32)TriCore_int_table);
  //EE_tc_endint_enable();
  
  /* Interrupt Vector Table Initialization: this should not be a "constructor"
     Called automatically by libos boot code ?!? */
  _init_vectab();

  _install_int_handler( EE_ISR2_ID_EE_tc_system_timer_handler,
    (handler_t)EE_tc_system_timer_handler,
    EE_ISR2_ID_EE_tc_system_timer_handler);

  _install_int_handler( EE_ISR2_ID_button_handler,
    (handler_t)button_handler,
    EE_ISR2_ID_button_handler);
