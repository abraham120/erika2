...
EE_UREG EE_SHARED_UDATA shared_slave1;
EE_UREG EE_SHARED_UDATA shared_slave2;
...
static StatusType       const test_as_errors_expected[ERROR_LENGTH] = {
  E_OS_ACCESS, E_OS_ACCESS, E_OS_ID, E_OS_ID, E_OS_NESTING_DEADLOCK,
...
int main(void)
{
  ...
  /* Errors */
  status = GetSpinlock(spinlock_2);
  test_assert(status == E_OS_ACCESS);
