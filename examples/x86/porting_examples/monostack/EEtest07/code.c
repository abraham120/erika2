...
void EE_pit_handler(void) {
  ...
  if (isr1_fired == 1) {
    EE_assert(EE_ASSERT_TIMER_FIRED, isr1_fired == 1, EE_ASSERT_TASK2_FIRED);
  }
...
TASK(Task2)
{
 ...
 if ((task2_fired % 5) == 1) {
    ...
    printf("\t\tTask2 Activating myself 6 times, although only 4 are allowed\n");
  ...
  printf("\t\tTask2 calling TerminateTask().\n");
  TerminateTask();
