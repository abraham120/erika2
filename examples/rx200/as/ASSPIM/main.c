...
int main(void)
{
	...
	EE_assert(EE_ASSERT_SPI_EB_SYNC_TX, (Spi_SyncTransmit(SPI_TEST_SEQ_0) == 
			E_OK), EE_ASSERT_SPI_EB_SYNC_SETUP);
	...
	if (Spi_GetSequenceResult(SPI_TEST_SEQ_0) == SPI_SEQ_OK) {
		EE_assert(EE_ASSERT_SPI_EB_SYNC_VALIDATE, (rx == SPI_TX_RX_DATA), 
