...
int main(void)
{
	...
	if ( Spi_GetSequenceResult(SPI_TEST_SEQ_0) == SPI_SEQ_OK ) {
		...
		EE_assert(EE_ASSERT_SPI_INT_READ_VALIDATE, 
				( rx == SPI_TEST_CHANNEL_0_DEFAULT_DATA ), 
	...
	EE_assert(EE_ASSERT_SPI_EB_INT_SETUP, 
			(Spi_SetupEB(SPI_TEST_CHANNEL_0_EB, NULL_PTR, &rx, SPI_EB_LENGTH) == 
					E_OK), EE_ASSERT_SPI_EB_INT_MODE);
	...
	EE_assert(EE_ASSERT_SPI_EB_POLL_SETUP,( 
			Spi_SetupEB(SPI_TEST_CHANNEL_0_EB, NULL_PTR, &rx, SPI_EB_LENGTH) == 
					E_OK), EE_ASSERT_SPI_EB_POLL_MODE);
	...
	EE_assert(EE_ASSERT_SPI_EB_SYNC_SETUP,
			(Spi_SetupEB(SPI_TEST_CHANNEL_0_EB, NULL_PTR, &rx, SPI_EB_LENGTH) == 
					E_OK), EE_ASSERT_SPI_SYNC_READ_VALIDATE);
