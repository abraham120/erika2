...
boolean cmp_buff(Spi_DataType *buff, Spi_NumberOfDataType len)
{
	...
	if ( len > SPI_LBM_CHANNEL_2_IB_N_BUFFERS ) {
		...
		for (i = 0; (i < len) && rv; i++) {
			...
			if ( buff[i] != SPI_LBM_CHANNEL_2_DEFAULT_DATA ) {
...
int main(void)
{
	...
	EE_assert(EE_ASSERT_SPI_INT_WRITE_IB, TRUE, EE_ASSERT_SPI_INTERRUPT_MODE);
	...
	EE_assert(EE_ASSERT_SPI_POLL_WRITE_IB, TRUE, EE_ASSERT_SPI_POLLING_MODE);
	...
	EE_assert(EE_ASSERT_SPI_EB_INT_SETUP, (Spi_SetupEB(SPI_LBM_CHANNEL_2_EB, 
			NULL_PTR, rx, SPI_LBM_CHANNEL_2_IB_N_BUFFERS)== E_OK), 
	...
	EE_assert(EE_ASSERT_SPI_SYNC_WRITE_IB, TRUE, EE_ASSERT_SPI_EB_POLL_VALIDATE);
	...
	EE_assert(EE_ASSERT_SPI_EB_SYNC_SETUP,(Spi_SetupEB(SPI_LBM_CHANNEL_2_EB, 
			NULL_PTR, rx, SPI_LBM_CHANNEL_2_IB_N_BUFFERS) == E_OK), 
