...
/*
 * Simple project to test Sci_GoToSleep(), Sci_Wakeup() and Sci_CheckWakeup()
 * services.
 *
...
enum EE_ASSERTIONS {
	...
	EE_ASSERT_SCI_INIT,
	EE_ASSERT_SCI_PRE_SLEEP,
	EE_ASSERT_SCI_TEST_SLEEP,
	EE_ASSERT_SCI_PRE_WAKEUP,
	EE_ASSERT_SCI_TEST_WAKEUP,
	EE_ASSERT_DIM
};
...
int main(void)
{
	...
	Sci_DisableRx(SCI_CHANNEL_0);

	EE_assert(EE_ASSERT_SCI_PRE_SLEEP,
				( Sci_GoToSleep(SCI_CHANNEL_0) == E_OK ),
				EE_ASSERT_SCI_INIT);

	EE_assert(EE_ASSERT_SCI_TEST_SLEEP,
				( Sci_CheckWakeup(SCI_CHANNEL_0) == E_NOT_OK ),
				EE_ASSERT_SCI_PRE_SLEEP);

	EE_assert(EE_ASSERT_SCI_PRE_WAKEUP, 
				( Sci_Wakeup(SCI_CHANNEL_0) == E_OK ),
				EE_ASSERT_SCI_TEST_SLEEP);

	EE_assert(EE_ASSERT_SCI_TEST_WAKEUP, 
				( Sci_CheckWakeup(SCI_CHANNEL_0) == E_OK ), 
				EE_ASSERT_SCI_PRE_WAKEUP);

	EE_assert_range(EE_ASSERT_FIN, TRUE, EE_ASSERT_SCI_TEST_WAKEUP);
