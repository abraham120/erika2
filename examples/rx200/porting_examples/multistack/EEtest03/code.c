 ...
 * Based on examples/cortex_mx/lm4f232xxxx/porting_examples_keil/EEtest3
 */
...
{	
	sw_counter++;
	EE_assert(EE_ASSERT_SWINT_ISR_FIRED, sw_counter == 1, EE_ASSERT_INIT);
	ActivateTask(Task1);
	while (tm0_counter < 1000);
	EE_systick_stop();
}
...
{	
	tm0_counter++;
	if (tm0_counter == 100) {
		EE_assert(EE_ASSERT_CMIA0_ISR_FIRED, tm0_counter == 100, 
			EE_ASSERT_SWINT_ISR_FIRED);
	}
	if (tm0_counter % 100 == 0)
		ActivateTask(Task2);
}
...
{
	task1_fired++;
	EE_assert(EE_ASSERT_TASK1_FIRED, task1_fired == 1, 
				EE_ASSERT_TASK2_TEN_INSTANCES );
	task1_ended++;
}
...
TASK(Task2)
{
	task2_fired++;
	if (task2_fired == 1)
		EE_assert(EE_ASSERT_TASK2_FIRED, task2_fired == 1, 
					EE_ASSERT_CMIA0_ISR_FIRED);
	else if (task2_fired >= 10)
			EE_assert(EE_ASSERT_TASK2_TEN_INSTANCES, task2_fired == 10,
						EE_ASSERT_TASK2_FIRED);
	task2_ended++;
}
...
int main(void)
{
	...
	counter++;
	EE_assert(EE_ASSERT_TASKS_ENDED, task1_ended && (task2_ended == 10),
				EE_ASSERT_TASK1_FIRED);
