...
{	
	...
	LCD_LOG_Init();
	LCD_LOG_SetHeader("Dining Philosopher Probl.");
	LCD_LOG_SetFooter("Erika RTOS and QP Framework DPP Demo");
	
	LCD_UsrLog("\r\nErika is the underlying RTOS");
	LCD_UsrLog("\r\nQEP %s", QEP_getVersion(), QF_getVersion());
	LCD_UsrLog("\r\nQF  %s", QEP_getVersion(), QF_getVersion());
	LCD_UsrLog("\r\n                             \r\n");
