...
TASK(TaskSeqOk)
{
  ...
  switch ( test ) {
    ...
    case 2:
      EE_assert(
	EE_ASSERT_SPI_EB_INT_SEQ_OK, TRUE, EE_ASSERT_SPI_EB_INT_JOB_END
      );
      break;

    case 3:
      EE_assert(
	EE_ASSERT_SPI_EB_POLL_SEQ_OK, TRUE, EE_ASSERT_SPI_EB_POLL_JOB_END
      );
      break;
...
int main(void)
{
  ...
  EE_assert(
    ...
    ( 
      Spi_SetupEB(SPI_TEST_CHANNEL_0_EB, NULL_PTR, &rx, SPI_EB_LENGTH) == E_OK
    ),
    EE_ASSERT_SPI_EB_INT_MODE
  );
