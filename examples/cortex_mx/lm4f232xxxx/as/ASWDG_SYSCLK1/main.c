...
/*
 * Simple project to test the System Clock WDG driver services using callback
 * and standard watchdog interrupt without system reset.
 *
...
TASK(BackgroundTask)
{
  ...
  EE_assert(
    EE_ASSERT_WDG_SLOW_MODE,
    ( Wdg_SysClk_SetMode(WDGIF_SLOW_MODE) == E_OK ),
    EE_ASSERT_WDG_INIT
  );
