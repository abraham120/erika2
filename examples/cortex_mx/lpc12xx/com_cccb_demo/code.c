...
TASK(Task1)
{
	...
	if (ReadFlag_data_in() == EE_COM_TRUE) {
		while (ReceiveMessage(RECEIVER2, (EE_UINT8 *)&data1) != E_COM_NOMSG) {
			ABS_Data += data1;
			data_rx_queued++;
		}
	}
...
TASK(Task2)
{
	...
	ReceiveMessage(RECEIVER1, (EE_UINT8 *)&data2);
	ABS_Data += data2;
	data_rx_unqueued++;
}
